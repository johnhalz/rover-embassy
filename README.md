...
The logger uses the `foxglove-flatbuffers` crate for type-safe FlatBuffer message handling. The Rust code and binary schema files are automatically generated by the crate from the official Foxglove schemas. The schema data for MCAP and WebSocket registration is provided by the `foxglove-flatbuffers::helpers::log_schema_bytes()` function.
